...
var Verify = new(verifyApi)

type verifyApi struct{}

func (c *verifyApi) LoadRouter(group *ghttp.RouterGroup) {
	group.Group("/verify", func(group *ghttp.RouterGroup) {
...
func (c *verifyApi) getList(r *ghttp.Request) {

	//获取QueryParam
	var req *dto.VerifyQuery
	...
	if total, result, code := service.Verify.SelectList(req); code != response.SUCCESS {
		response.Error(r).
			SetCode(code).
			SetMessage(response.
				CodeMsg(code)).Send()
...
func (c *verifyApi) postReview(r *ghttp.Request) {
	var req *dto.Review
	if err := r.Parse(&req); err != nil {
		response.Error(r).
			SetCode(response.PARAM_INVALID).
	...
	if code := service.Verify.Review(req); code != response.SUCCESS {
		response.Error(r).
			SetCode(code).
...
func (c *verifyApi) postRemove(r *ghttp.Request) {
	rule := "required"
	...
	if code := service.Verify.Remove(gconv.Int64s(ids)); code != response.SUCCESS {
