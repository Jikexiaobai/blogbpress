...
export default {
    ...
    methods: {
        async getData(){
            try {
               const res = await this.$axios.get(api.getAudioList,{params: this.queryParam})
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
        ...
        create(){
            this.$router.push({ name: "member-audio-create"})
        },
        view(e){
            this.$router.push({ name: "audio-id",params:{id:e}})
        },
        edit(e){
            this.$router.push({ name: "member-audio-edit",
                query:{
                    id:e,
                }
            })
        ...
        async postDelete(formData){
            try {
                const res = await this.$axios.post(api.postAudioRemove,formData)
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
