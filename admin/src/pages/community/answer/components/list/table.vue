<template>
    <div class="container">
        <a-space class="container-action" size="large">
            ...
            >
                <a-button v-action="'/answer/review'" :disabled="selectedRowKeys.length > 0 ? false :true">
                    批量审核
                </a-button>
            </a-popconfirm>
            <a-button
                v-action="'/answer/recover'"
                v-if="status != 4"
                :disabled="selectedRowKeys.length > 0 ? false :true" 
        ...
        <span slot="content" slot-scope="content" class="content" >
            <div v-html="content"></div>
        </span>
        <span slot="action" slot-scope="text, record">
            <!-- <a @click="edit(record.id)"   v-if="status != 4">{{$t('edit')}}</a> -->
            <a @click="reduction(record.id)"  v-action="'/answer/reduction'"  v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)"  v-action="'/answer/remove'"  v-if="status == 4">删除</a> 
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item v-action="'/answer/review'"  v-if="record.status == 1">
                        <a-popconfirm
                            title="审核"
                    ...
                    <a-menu-item v-action="'/answer/recover'">
...
<script>
import { Remove,Review,Reduction,Recover } from '@/api/answer'
export default {
    props:{
        list:{
            type:Array,
